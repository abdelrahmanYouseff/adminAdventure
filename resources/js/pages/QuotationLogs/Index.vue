...
import { ChevronLeft, ChevronRight, FileSpreadsheet, Search } from 'lucide-vue-next';
    ...
    <div class="flex min-w-0 flex-1 flex-col gap-6 overflow-x-hidden p-3 pb-8 sm:p-6" dir="rtl">
        <section class="rounded-[2rem] bg-slate-950 px-6 py-8 text-white shadow-xl sm:px-8">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <p class="text-xs font-semibold tracking-[0.18em] text-sky-200/80">QUOTATION AUDIT</p>
                    ...
                    <p class="mt-2 max-w-2xl text-sm leading-relaxed text-slate-300">
                        كل عروض الأسعار الحالية. المنشئ والمعدّل يظهران بعد تفعيل السجل، والعروض الأقدم تظهر بدون اسم المستخدم لأنه لم يكن يُسجَّل سابقاً.
                    </p>
                </div>
                <p class="text-sm text-slate-400">{{ formatInteger(quotations.total) }} عرض سعر</p>
            </div>
        </section>
        ...
        <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            <button type="button" class="rounded-3xl bg-white p-5 text-right shadow-sm ring-1 ring-slate-200" @click="setFilter('all')">
                <p class="text-xs font-semibold text-slate-400">كل العروض</p>
                ...
                <form class="w-full max-w-md" @submit.prevent="applyFilters(1)">
                    <label class="flex h-10 items-center gap-2 rounded-full border border-transparent bg-gray-100 px-3.5 text-gray-400 transition focus-within:border-blue-300 focus-within:bg-white focus-within:ring-2 focus-within:ring-blue-100">
                        ...
                        <input
                            v-model="searchQuery"
                            type="search"
                            placeholder="ابحث برقم العرض أو العميل أو اسم المستخدم..."
                ...
                <table class="w-full min-w-[980px] border-collapse text-center text-sm">
                    <thead>
                        <tr class="border-b border-gray-100 bg-gray-50/80">
                            <th class="px-4 py-3.5 text-center text-[13px] font-semibold text-gray-700">رقم العرض</th>
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td class="px-3 py-3.5 text-center font-medium text-gray-900">
                                {{ quotation.customer_name || 'بدون اسم عميل' }}
                            </td>
                            <td class="px-3 py-3.5 text-center text-gray-700">
                                {{ actorLabel(quotation.created_by) }}
                            </td>
                            <td class="px-3 py-3.5 text-center tabular-nums text-gray-600" dir="ltr">
                                {{ quotation.created_at ? formatDateTime(quotation.created_at) : '—' }}
                            </td>
                            <td class="px-3 py-3.5 text-center text-gray-700">
                                <template v-if="quotation.updated_by">
                                    <p>{{ actorLabel(quotation.updated_by) }}</p>
                                    <p v-if="quotation.updates.length > 1" class="mt-0.5 text-xs text-amber-700">
                                        {{ quotation.updates.length }} تعديلات
                                    </p>
                                </template>
                                <span v-else class="text-gray-400">لا يوجد تعديل</span>
                            </td>
                            <td class="px-3 py-3.5 text-center tabular-nums text-gray-600" dir="ltr">
                                {{ quotation.updated_at ? formatDateTime(quotation.updated_at) : '—' }}
                            </td>
                            <td class="px-4 py-3.5 text-center">
                                <Link
        ...
        <div v-if="quotations.last_page > 1" class="flex items-center justify-between gap-4 rounded-3xl bg-white px-5 py-4 shadow-sm ring-1 ring-slate-200">
            <p class="text-sm text-slate-500">
                عرض {{ quotations.from ?? 0 }} - {{ quotations.to ?? 0 }} من {{ quotations.total }}
            </p>
            <div class="flex items-center gap-1">
                <button
                    ...
                    @click="applyFilters(quotations.current_page - 1)"
                >
                ...
                <template v-for="page in pageNumbers" :key="String(page)">
                    ...
                    <button
                        ...
                        :class="page === quotations.current_page ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-100'"
