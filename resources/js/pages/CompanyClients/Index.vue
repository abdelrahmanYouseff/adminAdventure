...
function destroyClient(client: CompanyClient) {
    if (!confirm(`حذف شركة «${client.company_name}»؟`)) {
        return;
    }

    router.delete(route('company-clients.destroy', client.id), {
        preserveScroll: true,
    });
}

function toggleClient(clientId: number) {
    if (expandedId.value === clientId) {
        expandedId.value = null;
        expandedQuotationId.value = null;
        return;
    }

    expandedId.value = clientId;
    expandedQuotationId.value = null;
}
...
<template>
    ...
    <div class="flex min-w-0 flex-1 flex-col gap-4 overflow-x-hidden p-3 pb-[max(1rem,env(safe-area-inset-bottom))] sm:gap-6 sm:p-6">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <h1 class="text-xl font-bold tracking-tight sm:text-3xl">عملاء الشركات</h1>
                <p class="mt-1 text-sm text-muted-foreground">
                    اضغط على الشركة لعرض عروض الأسعار المرتبطة بها
                </p>
            </div>
            <Button
                v-if="!formOpen"
                    ...
                    <table class="w-full border-collapse">
                        <thead>
                            <tr class="border-b bg-muted/40 text-sm">
                                ...
                                <th class="px-4 py-3 text-right font-semibold">المسؤول</th>
                                <th class="px-4 py-3 text-right font-semibold">الجوال</th>
                                <th class="px-4 py-3 text-right font-semibold">عروض الأسعار</th>
                                <th class="px-4 py-3 text-right font-semibold">تاريخ الإضافة</th>
                        ...
                        <tbody>
                            <tr v-if="clients.length === 0">
                                <td colspan="7" class="px-4 py-12 text-center text-sm text-muted-foreground">
                                    لا يوجد عملاء شركات بعد. اضغط «إضافة شركة» للبدء.
                                </td>
                            </tr>
                            ...
                            <template v-for="client in clients" :key="client.id">
                                ...
                                <tr v-if="expandedId === client.id">
                                    <td colspan="7" class="bg-muted/10 px-3 py-4 sm:px-6">
                                        <div v-if="client.quotations.length === 0" class="rounded-xl border border-dashed px-4 py-8 text-center text-sm text-muted-foreground">
                                            لا توجد عروض أسعار مرتبطة بهذه الشركة بعد.
                                        </div>

                                        <div v-else class="space-y-3">
                                            <p class="text-sm font-medium text-muted-foreground">
                                                عروض الأسعار المرسلة لـ «{{ client.company_name }}»
                                                ({{ formatInteger(client.quotations_count) }})
                                            </p>

                                            <div
                                                v-for="quotation in client.quotations"
                                                :key="quotation.id"
