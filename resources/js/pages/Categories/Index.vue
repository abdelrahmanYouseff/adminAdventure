...
<template>
    <Head title="الأصناف حسب البراند" />

    <div class="flex min-w-0 flex-1 flex-col gap-6 p-4 sm:p-6" dir="rtl">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <h1 class="text-2xl font-bold tracking-tight sm:text-3xl">براندات الأصناف</h1>
                <p class="mt-1 text-sm text-muted-foreground">اختر البراند لعرض وإدارة أصناف المنتجات الخاصة به</p>
            </div>
        ...
        <div v-else class="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
            <Link
                ...
                :key="brand.id"
                :href="route('categories.brand.show', brand.slug)"
            ...
            >
                <div class="relative flex h-48 items-center justify-center overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900">
                    ...
                    />
                    <div
                        v-else-if="brand.categories.some((category) => categoryImage(category))"
                        class="grid h-full w-full grid-cols-2"
                    >
                        <div
                            v-for="category in brand.categories.slice(0, 4)"
                            :key="category.id"
                            class="overflow-hidden border border-background/60 bg-muted"
                        >
                            <img
                ...
                <div class="p-5">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <div class="flex shrink-0 items-center gap-1 rounded-lg bg-muted px-2.5 py-1.5 text-sm font-semibold">
                            ...
                            {{ formatInteger(brand.categories_count) }}
                        </div>
                    </div>
                    <div class="mt-4 border-t pt-3 text-sm font-medium text-primary">عرض أصناف البراند</div>
                </div>
            </Link>
        </div>
    </div>
